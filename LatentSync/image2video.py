    ...
    for job in all_jobs:
        ...
        for source in sources:
            for task in base_tasks:
                ...
                # idx 為 boolean：True -> 從 part_folder 取數字並補零；False/缺省 -> 不加 idx
                idx_flag = bool(task.get('idx', False))
                idx_str = ''
                if idx_flag:
                    digits = ''.join(ch for ch in part_folder if ch.isdigit())
                    if digits:
                        idx_str = digits.zfill(2)  # e.g., "part3" -> "03"

                filename = task['filename']
                out_name = f'{source}_{idx_str}_{filename}.mp4' if idx_str else f'{source}_{filename}.mp4'
    ...
    all_jobs = [
        {
            ...
            "base_tasks": [
                # {"part_folder": "part1", "idx": True, "filename": "draw_cards"},
                {"part_folder": "part2", "idx": True, "filename": "open_cards"},
                # {"part_folder": "part3", "idx": True, "filename": "collect_cards"},
            ],
        },
        # {
        #     "sources": ['clip'],
        #     "base_tasks": [
        #         {"part_folder": "game_start", "idx": False, "filename": "game_start"},
        #         {"part_folder": "loop", "idx": False, "filename": "loop"},
        #     ],
        # },
    ]
